...
export const masks = {
  linear: ({ direction = "to-right", opacities = [1, 0], positions, rotate = 0 }: LinearMaskParams): string => {
    ...
    const stops = positions.map(
      (p, i) => `rgba(255, 255, 255, ${opacities[i]}) ${typeof p === "number" ? `${p * 100}%` : p}`,
    );

    return `linear-gradient(${baseRotation + rotate}deg, ${stops.join(", ")})`;
  },
  customLinear: ({ opacityFn, stops = 10, direction = "to-right", rotate = 0 }: CustomLinearMaskParams) => {
    const baseRotation = BASE_ROTATIONS[direction];
    const positions = Array.from({ length: stops }, (_, i) => i / (stops - 1));
    const opacities = positions.map(opacityFn);
    const gradientStops = positions.map((pos, index) => `rgba(255, 255, 255, ${opacities[index]}) ${pos * 100}%`);
  ...
  circle: ({ opacities = [1, 0], positions }: CircleMaskParams): string => {
    ...
    const stops = positions.map((p, i) => `rgba(255, 255, 255, ${opacities[i]}) ${p * 100}%`);
...
export const Mask = forwardRef<HTMLDivElement, MaskProps>(({ image, size, ...props }, ref) => {
  return (
    <motion.div
      {...props}
      ref={ref}

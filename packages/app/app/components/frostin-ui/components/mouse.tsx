import { type HTMLMotionProps, type SpringOptions, useSpring, useTransform } from "framer-motion";
import { motion } from "framer-motion";
import { type MotionValue, useMotionValue, useVelocity } from "framer-motion";
import {
  type FC,
...
import type { Loose } from "../utils/types";
...
const useMousePosition = ({ defaultX = -1000, defaultY = -1000 }: { defaultX?: number; defaultY?: number } = {}) => {
...
export const MouseProvider = ({
  ...
  const activeTarget = targets[targets.length - 1] as MouseTargetInfo | undefined;
...
export const MouseTarget = ({ children, targetId, data = {}, ...otherProps }: MouseTargetProps) => {
...
export const MouseSmoothing: FC<PropsWithChildren<MouseSmoothingProps>> = ({ position, velocity, children }) => {
